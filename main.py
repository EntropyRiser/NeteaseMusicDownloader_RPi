...
-------------------------------------------------------\n" \
		...
		if not os.path.exists("/home/pi/Music/Download"):
			os.mkdir('/home/pi/Music/Download')
	...
	def get(self,values):
		...
		for x in values:
			...
			if not os.path.exists("/home/pi/Music/Download/" + x['name'] + '.mp3'):
				self.output_text.AppendText('***** ' + x['name'] + '.mp3 ***** Downloading...\n')
				
				url = 'http://music.163.com/song/media/outer/url?id=' + x['id'] + '.mp3'
				try: 
					# urllib.request.urlretrieve(url,'/home/pi/Music/Download/' + x['name'] + '.mp3')
					self.saveFile(url,'/home/pi/Music/Download/' + x['name'] + '.mp3')
					downNum = downNum + 1
				except:
					x = x - 1
